...
### 6/22/2019 - Using local storate to persist data
We use local storage `localStorage()` to store our order data. This is finally one I've used before. Local storage takes key / value pairs and you can use `.setItem()` and `.getItem()` to store and retrieve our order info. We are storing the item in our cart, which is stored in state, which is an object. So since local storage takes a string, we have to use `JSON.stringify()` and `JSON.parse()` to take the object and make a string representation so we could store it in localStorage and then transform it back to an object once we retrieve it. Another one I've used before! Yay!

So the localStorage() is set on another React lifecycle event `componentDidUpdate()`.  For more information -> [here](https://reactjs.org/docs/react-component.html). So the interesting thing was dealing with the timing between gettihandleChange = (event) => {
    ...
    };ng data from local storage, which has a fish object, versus getting the inventory data from firebase. Since it takes more time to make the call to firebase to retrieve the data than it does to update state from local storage, an error occurs. So to fix this, where we check to see if a fish is available in the order component, we have to also check to see if a fish exists, and if not, return null. So we will render a blank order component, but as soon as firebase sends back the data, that will update state, which will trigger componentDidUpdate(), which then calls localStorage to retrieve the order information (assuming that there is anything to be returned). 

The other question that I had was really about `componentDidUpdate()` was 'which component are we talking about here?' as we have an order component that can update, an inventory component and a fish component. So how do we tell componentDidUpdate() which component is updating? Answer, in this case we don't need to. We call componentDidUpdate() from our App component which manages state for our order component. Updating state (adding a new fish to the order), triggers componentDidUpdate(), which saves the data to local storage. 
